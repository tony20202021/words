...
if __name__ == "__main__":
    INPUT_FILE_JSON = "/home/tony/repos/words/words/radicals_names/radicals_names_5/data/words_Китайский_20251002_055605.json.new_radicals.json.tones.json"
    ...
    for entry in data["words"]:
        ...
        for char in word_foreign:
            ...
            if char not in chars_ref:
                chars_ref[char] = []
            chars_ref[char].append(entry)
            ...
            if char in chars_ref:
                for ref in chars_ref[char]:
                    if ref["word_number"] != entry["word_number"]:
                        words_formatted.append(f"<i>[#{ref['word_number']}]</i>{ref['word_foreign']} [{ref['transcription']}] {ref['translation']}")

            if len(words_formatted) > 0:
                words_all = "\n".join(words_formatted)
                references_formatted.append(f"{reference_text}\n{words_all}")
            else:
