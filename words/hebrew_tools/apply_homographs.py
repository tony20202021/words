...
Как устроено
------------
Основное чтение остаётся в word_foreign и transcription — карточка внутренне
непротиворечива. Остальные чтения дописываются в перевод, каждое со своей
огласовкой, транскрипцией и значением:

    инстинкт, влечение
    ⚠ то же написание читается иначе: יָצַר (яцАр) — создал

Схему БД менять не потребовалось: перевод и так свободный текст, и все три
клиента показывают его как есть.
...
MARK = "⚠ то же написание читается иначе:"


def build_translation(readings: list[dict]) -> str:
    """Перевод основного чтения плюс явное указание остальных."""
    main, rest = readings[0], readings[1:]
    lines = [main["sense"]]
    for r in rest:
        lines.append(f"{MARK} {r['niqqud']} ({r['translit_ru']}) — {r['sense']}")
    return "\n".join(lines)
