...
class StyleDefinitions:
    ...
    def add_custom_style(self, style_definition: StyleDefinition):
        """
        Добавляет пользовательский стиль.
        
        Args:
            style_definition: Определение стиля
        """
        self.styles[style_definition.name] = style_definition
        logger.info(f"Added custom style: {style_definition.name}")
    ...
    def remove_style(self, style_name: str) -> bool:
        ...
        Returns:
            bool: True если стиль был удален
        """
        if style_name in self.styles:
            del self.styles[style_name]
            logger.info(f"Removed style: {style_name}")
            return True
        return False
    
    def update_style(self, style_name: str, updates: Dict[str, Any]) -> bool:
        """
        Обновляет существующий стиль.
        ...
        Returns:
            bool: True если стиль был обновлен
        """
        if style_name not in self.styles:
            return False
        ...
        for field, value in updates.items():
            if hasattr(style_def, field):
                setattr(style_def, field, value)
        
        logger.info(f"Updated style: {style_name}")
        return True
    
    def get_style_comparison(self, style1: str, style2: str) -> Dict[str, Any]:
        """
        Сравнивает два стиля.
        ...
        Returns:
            Dict[str, Any]: Сравнение стилей
        """
        def1 = self.get_style_definition(style1)
        def2 = self.get_style_definition(style2)
        
        comparison = {
            "style1": {
                "name": def1["name"],
                "display_name": def1["display_name"]
            },
            "style2": {
                "name": def2["name"], 
                "display_name": def2["display_name"]
            },
            "differences": {
                "steps": def1["recommended_steps"] - def2["recommended_steps"],
                "cfg": def1["recommended_cfg"] - def2["recommended_cfg"],
                "controlnet_weights": {
                    control_type: def1["controlnet_weights"][control_type] - def2["controlnet_weights"][control_type]
                    for control_type in def1["controlnet_weights"].keys()
                }
            },
            "common_modifiers": list(set(def1["style_modifiers"]) & set(def2["style_modifiers"])),
            "unique_modifiers": {
                "style1": list(set(def1["style_modifiers"]) - set(def2["style_modifiers"])),
                "style2": list(set(def2["style_modifiers"]) - set(def1["style_modifiers"]))
            }
        }
    ...
    def get_style_stats(self) -> Dict[str, Any]:
        """
        Возвращает статистику по стилям.
        
        Returns:
            Dict[str, Any]: Статистика стилей
        """
        total_styles = len(self.styles)
        
        # Анализируем параметры стилей
        avg_steps = sum(style.recommended_steps for style in self.styles.values()) / total_styles
        avg_cfg = sum(style.recommended_cfg for style in self.styles.values()) / total_styles
        
        # Подсчитываем модификаторы
        all_modifiers = []
        for style in self.styles.values():
            all_modifiers.extend(style.style_modifiers)
        
        unique_modifiers = len(set(all_modifiers))
        
        return {
            "total_styles": total_styles,
            "available_styles": list(self.styles.keys()),
            "avg_recommended_steps": avg_steps,
            "avg_recommended_cfg": avg_cfg,
            "total_unique_modifiers": unique_modifiers,
            "most_common_sizes": [(1024, 1024)],  # Все используют одинаковый размер
            "styles_with_postprocessing": len([s for s in self.styles.values() if s.postprocessing_filters])
        }
