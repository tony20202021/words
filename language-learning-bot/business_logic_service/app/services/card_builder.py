...
def build_card(session: Dict[str, Any], word: Dict[str, Any], show_answer: bool) -> Dict[str, Any]:
    ...
    # Большой плашки «⏱ Вы знали это слово» здесь больше нет. Ту же вещь уже
    # говорит компактный бейдж в шапке — «знал · N дн.» либо «не знал», — а два
    # сообщения об одном и том же на одном экране лишние. Онлайн большой плашки
    # и не было: условие смотрело на значения после ответа, а «Не знаю» их
    # обнуляет раньше, чем строится карточка. Видно её было только офлайн, где
    # карточка рисуется заранее, — то есть она существовала как артефакт.
