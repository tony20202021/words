...
`major*10000 + minor*100 + patch` (напр. 3.0.74 → 30074) — править `build.gradle` не нужно.
...
Помимо обычных, у каждого клиента есть тесты **последовательностей** —
`tests/test_scenarios.py` в `web_frontend` и `telegram_bot`,
`OfflineScenarioTest.kt` в Android.

Они нужны потому, что баги живут в переходах, а не в отдельных функциях:
«Начать заново» возвращало на то же слово, настройка сбрасывала язык, отказ
сервера оставлял пустой экран. Одношаговый тест такого не видит.

Правила, выведенные из практики:

- проверять **порядок** вызовов, а не только факт (рестарт обязан закрыть
  сессию до открытия новой);
- проверять, что **состояние изменилось**, а не что ответ 200;
- обязательно закрывать сценарий **отказа сервера** — оба найденных
  «висящих экрана» были именно там;
- общий пролог держать в фикстуре, а не повторять в каждом сценарии.
