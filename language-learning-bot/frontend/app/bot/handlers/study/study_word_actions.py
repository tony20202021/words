...
async def process_word_know(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        # Получаем интервал до следующего повторения
        check_interval = result.get("check_interval", 0)
        next_check_date = result.get("next_check_date")
        
        interval_message = ""
        if check_interval and next_check_date:
            formatted_date = format_date(next_check_date)
            interval_message = f"Следующее повторение: через {check_interval} дн. ({formatted_date})\n\n"
        
        # Создаем кнопку для перехода к следующему слову
        keyboard = InlineKeyboardBuilder()
        keyboard.button(
            text="➡️ Перейти к следующему слову",
            callback_data="confirm_next_word"
        )
        # TODO добавить кнопку "нет, все-таки не знаю" - по ней делать show_word
