...
Дефекты, которые УЖЕ починены, живут отдельно — в
[docs/development/defect_log.md](development/defect_log.md).
Здесь только незакрытое: смешав то и другое, нельзя посчитать ни
оставшуюся работу, ни то, сколько дефектов доходит до пользователя.
