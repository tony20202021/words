...
11. [Методы для экспорта данных](#методы-для-экспорта-данных)
12. [Обработка ошибок](#обработка-ошибок)
13. [Примечания](#примечания)
14. [Обновления во фронтенде](#обновления-во-фронтенде)
...
### export_words_by_language(language_id, format, start_word, end_word, timeout_multiplier)

Экспорт слов для конкретного языка в различных форматах.

**Параметры:**
- `language_id` (str): ID языка для экспорта
- `format` (str, опционально): Формат экспорта - "xlsx" (по умолчанию), "csv", "json"
- `start_word` (int, опционально): Начальный номер слова для фильтрации (включительно)
- `end_word` (int, опционально): Конечный номер слова для фильтрации (включительно)
- `timeout_multiplier` (int, опционально): Множитель для таймаута запроса (по умолчанию: 5). Экспорт может занимать время для больших объемов данных.

**Возвращает:**
- Словарь с результатами согласно стандартной структуре ответа
- В случае успеха, `result` содержит бинарные данные файла для сохранения/отправки

**Структура экспорта:**
- **Excel/CSV**: Столбцы - № | Слово | Перевод | Транскрипция
- **JSON**: Структурированный JSON с информацией о языке и списком слов

```python
# Экспорт всех слов языка в Excel
...
if export_response["success"]:
    ...
    # file_data содержит бинарные данные файла
    
    # Сохранение файла
    with open("exported_words.xlsx", "wb") as f:
        f.write(file_data)
    
    print("Слова успешно экспортированы в Excel")
else:
    error_msg = export_response.get("error", "Неизвестная ошибка")
    print(f"Ошибка экспорта: {error_msg}")

# Экспорт слов в диапазоне в CSV формате
export_response = await api_client.export_words_by_language(
    language_id="123abc",
    format="csv", 
    start_word=1,
    end_word=100,
    timeout_multiplier=10  # Увеличенный таймаут для большого файла
)
...
if export_response["success"]:
    ...
    await bot.send_document(
        ...
        document=BufferedInputFile(file_data, filename="words_1-100.csv"),
        caption="Экспорт слов 1-100"
    )
    print("CSV файл отправлен администратору")
...
if export_response["success"]:
    import json
    json_data = json.loads(export_response["result"].decode('utf-8'))
    
    language_info = json_data["language"]
    words = json_data["words"]
    
    print(f"Экспортировано {len(words)} слов для языка '{language_info['name_ru']}'")
    for word in words[:5]:  # Показать первые 5 слов
        print(f"  {word['word_number']}: {word['word_foreign']} - {word['translation']}")
```

**Примечания по использованию:**
- Метод возвращает бинарные данные файла, готовые для сохранения или отправки
- Поддерживает фильтрацию по диапазону номеров слов
- Автоматически генерирует имена файлов с меткой времени
- Рекомендуется использовать увеличенный таймаут для экспорта больших объемов данных
- Excel файлы имеют автоматически настроенную ширину колонок
- CSV файлы сохраняются в UTF-8 с BOM для совместимости с Excel
...
1. Большинство методов API клиента возвращают результат в ключе `"result"` в виде:
   ...
   - Бинарных данных (bytes) для файлов экспорта
...
5. **Новое**: Метод `export_words_by_language` возвращает бинарные данные файла, которые можно сохранить локально или отправить через Telegram Bot API.
