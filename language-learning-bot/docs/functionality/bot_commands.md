...
### Важные замечания по организации роутеров

При организации роутеров и обработчиков в aiogram 3.x важно учитывать следующие моменты:

1. **Порядок включения роутеров**: роутеры включаются последовательно, и первый подходящий обработчик перехватывает сообщение. Например:
   ```python
   # Правильный порядок:
   main_router.include_router(cancel_router)  # Сначала команды отмены
   main_router.include_router(state_router)   # Затем обработчики состояний

Приоритет обработчиков: для критически важных команд (например, /cancel) следует использовать более высокий приоритет:
python@router.message(Command("cancel"), flags={"priority": 100})  # Высокий приоритет

Роутеры для состояний FSM: обработчики состояний FSM должны следовать после обработчиков команд, чтобы команды вроде /cancel всегда работали.
