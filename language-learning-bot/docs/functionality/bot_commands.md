...
| `/export` | **НОВОЕ:** Экспорт слов языка в различных форматах |
...
- **Получать сообщения** - включить/отключить получение уведомлений от администраторов
  - Влияет на получение системных уведомлений
  - Объявлений о новых функциях
  - Технических сообщений
  - По умолчанию включено для всех новых пользователей
...
- **"📥 Экспорт слов"** - **НОВОЕ:** экспорт словаря в Excel/CSV/JSON

#### **НОВОЕ:** Экспорт слов:
- **"📊 Excel (.xlsx)"** - экспорт в формате Excel для просмотра/редактирования
- **"📄 CSV (.csv)"** - экспорт в универсальном CSV формате
- **"🔧 JSON (.json)"** - экспорт в JSON для программной обработки
- **"📋 Все слова"** - экспорт полного словаря
- **"🔢 Указать диапазон"** - экспорт определенного диапазона слов (например, 1-1000)

#### **НОВОЕ:** Рассылка сообщений:
- **"📤 Сообщение всем пользователям"** - отправка уведомлений всем пользователям
- **Ввод текста сообщения** - создание текста для рассылки
- **Подтверждение отправки** - просмотр количества получателей и подтверждение
- **Отслеживание прогресса** - мониторинг процесса доставки в реальном времени
- **Статистика доставки** - просмотр результатов рассылки
...
- **"Назначить администратором"** - предоставить административные права
- **"Просмотр статистики пользователя"** - детальная информация об активности
...
- **Экспорт данных** - **НОВОЕ:** выгрузка словарей для резервного копирования и анализа
- **Массовые уведомления** - **НОВОЕ:** рассылка сообщений от администраторов

### **НОВОЕ:** Процесс рассылки сообщений (только для администраторов)

#### 1. Инициация рассылки:
- **"📤 Сообщение всем пользователям"** - начать создание рассылки

#### 2. Создание сообщения:
- **Ввод текста** - написать сообщение для отправки (до 4096 символов)
- **Поддержка форматирования** - использование жирного текста, курсива и других стилей Telegram

#### 3. Подтверждение рассылки:
- **Просмотр текста** - проверка сообщения перед отправкой
- **Количество получателей** - показ числа пользователей, которые получат сообщение
- **"✅ Отправить"** - подтверждение рассылки
- **"❌ Отменить"** - отмена операции

#### 4. Отслеживание прогресса:
- **Обновляемый счетчик** - показ количества отправленных сообщений в реальном времени
- **Статистика ошибок** - отображение неуспешных доставок
- **Процентный индикатор** - прогресс выполнения рассылки

#### 5. Финальная статистика:
- **Общее количество** - итоговые цифры по рассылке
- **Успешные доставки** - количество успешно отправленных сообщений
- **Ошибки доставки** - список пользователей с неуспешной доставкой
- **Возврат в админ-меню** - автоматическое возвращение к административным функциям
...
- **"/cancel"** - универсальная команда отмены для любого состояния

### Отладка и техническая информация

- **Показ технической информации** - отображение ID слов, состояний FSM и других данных для отладки
- **Логирование действий** - автоматическая запись всех действий пользователя для анализа
- **Обработка ошибок** - автоматическое восстановление после сбоев с сохранением контекста
- **Мониторинг рассылок** - **НОВОЕ:** логирование всех административных уведомлений